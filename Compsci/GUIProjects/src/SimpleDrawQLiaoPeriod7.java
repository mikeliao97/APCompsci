 ...
 * I thought the saving and opening the file would be very simple, oh boy
 * was I wrong. For my opening the file I chose to use StringTokenizer
 * to get height and width by parsing the string into an int. Then I would
 * reiterate through and store the three rgb values into an color array. 
 * after u store the rgb values into a color array. I would attemp
 * to draw the picture with my color array. For the save method
 * I used the color array grid[][] to get teh current colors and their
 * blue green and red values with getRed getBlue() getGreen(). Then I used
 * fileWriter to write to the file
 * ONE THING I DID NOT COMPLETE WAS THAT I HAD NO CONTROL OVER WHERE TO
 * SAVE IT. I tried to use FileChooser and showSaveDialog but for some reaso
 * it just wouldn't work. In the end I stored into a file called output.ppm.
 * with the red green and blue values from  grid[][]. The last component of
 * this lab was the part involving the repaint method. This was pretty simple
 * basically whenever the user drew something I stored it into a color array.
 * i put a drawing method into the repaint method that would draw with
 * the colors from the color array when it called the repaint method. This made
 * it so that repaint() would not clear my drawings.
 *
 */
 


public class SimpleDrawQLiaoPeriod7 {
